...
public abstract class StandOutWindow extends Service {
	...
	public static void toggleNoti(Context context,
			Class<? extends StandOutWindow> cls, int id, boolean show) {
	...
	public static Intent getToggleNoticationIntent(Context context,
			Class<? extends StandOutWindow> cls, int id, boolean showNotifi) {
		String action = ACTION_TOGGLE_NOTIFICATION;
		return new Intent(context, cls).putExtra("id", id)
				.putExtra("show_notification", showNotifi).setAction(action);
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null) {
			...
			if (ACTION_SHOW.equals(action) || ACTION_RESTORE.equals(action)) {
				...
				int fromId = intent.getIntExtra("fromId", DEFAULT_ID);
				boolean show = intent
						.getBooleanExtra("show_notification", true);
		...
		if (showNoti) {
			...
			if (notification != null) {
				...
				if (!startedForeground) {
					...
					startForeground(getClass().hashCode()
				...
				if (!startedForeground) {
					throw new RuntimeException(
							"Your StandOutWindow service must"
	...
	public String getPersistentNotificationTitle(int id) {
		return getResources().getString(R.string.notification_title,
				getAppName());
	}
	...
	public PopupWindow getDropDown(final int id) {
		...
		items.add(new DropDownListItem(
				(sThemeIndex == 0) ? R.drawable.ic_dlg_close_orange
						: R.drawable.ic_dlg_close, getResources().getString(
	...
	public final synchronized Window show(int id) {
		...
		boolean isShowNoti = mSharedPreferences.getBoolean(
				"prefs_key_capture_notification", true);
