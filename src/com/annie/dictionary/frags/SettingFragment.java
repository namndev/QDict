...
public class SettingFragment extends PreferenceFragment implements Def,
		OnPreferenceChangeListener {

	private SharedPreferences mSharedPreferences;
	private Preference mPrefSource;
	private ListPreference mFontPreference, mThemePreference,
			mMaxFavPreference;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSharedPreferences = activity.getSharedPreferences(APP_NAME,
		...
		mTTSPref = (CheckBoxPreference) findPreference(getResources()
				.getString(R.string.prefs_key_using_tts));
		mTTSPref.setOnPreferenceChangeListener(this);
		mNotifPref = (CheckBoxPreference) findPreference(getResources()
				.getString(R.string.prefs_key_capture_notification));
	...
	public View onCreateView(LayoutInflater paramLayoutInflater,
		...
		View view = super.onCreateView(paramLayoutInflater, paramViewGroup,
				paramBundle);
		mFont = Utils.getFont(activity, mSharedPreferences.getString(
				Def.PREF_KEY_FONT, Def.DEFAULT_FONT));
		applyFont(mFont);
		return view;
	}
	...
	private void initInfo() {
		if (mSpeechEng == null) {
			mSpeechEng = DictSpeechEng.getInstance(activity
					.getApplicationContext());
		}
		if (TextUtils.isEmpty(mFontPreference.getEntry())) {
			mFontPreference.setValueIndex(3);
		}
		boolean tts = mSharedPreferences.getBoolean(
		...
		mMaxFavPreference.setSummary(getString(
				R.string.prefs_title_max_favorite_word_summary,
				mMaxFavPreference.getEntry().toString()));
		// service
		mUseCapture.setChecked(mSharedPreferences.getBoolean(
				getString(R.string.prefs_key_using_capture), false));
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		if (preference == mFontPreference) {
			...
			mSharedPreferences.edit()
					.putString(PREF_KEY_FONT, newValue.toString()).apply();
			Intent intent = new Intent("intent.stavira.FOR_MAIN");
			intent.putExtra("svn_refesh_option", Def.PREF_KEY_FONT);
			activity.sendBroadcast(intent);
			mFont = Utils.getFont(activity, newValue.toString());
			applyFont(mFont);
			return true;
			...
			mSharedPreferences.edit()
					.putInt("prefs_key_max_recent_word", maxFav).apply();
			mMaxFavPreference.setSummary(getString(
					R.string.prefs_title_max_favorite_word_summary,
			...
			mSharedPreferences
					.edit()
					.putBoolean("prefs_key_capture_notification",
							(Boolean) newValue).apply();
			mNotifPref.setChecked((Boolean) newValue);
			if (QDictService.RUNNING) {
				StandOutWindow.toggleNoti(getActivity(), QDictService.class,
						StandOutWindow.DEFAULT_ID, (Boolean) newValue);
			}
			return true;
			...
			if (newValue instanceof Boolean) {
				...
				editor.putBoolean(getString(R.string.prefs_key_using_capture),
						boolVal);
				editor.apply();
				Intent intent = new Intent(MainActivity.ACTION_UPDATE_UI);
	...
	private void convertPreferenceToUseCustomFont(Preference somePreference) {
		...
		if (somePreference.getTitle() != null) {
			ss = new SpannableStringBuilder(somePreference.getTitle()
					.toString());
			ss.setSpan(customTypefaceSpan, 0, ss.length(),
					Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
			somePreference.setTitle(ss);
		}

		if (somePreference.getSummary() != null) {
			ss = new SpannableStringBuilder(somePreference.getSummary()
					.toString());
			ss.setSpan(customTypefaceSpan, 0, ss.length(),
					Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
