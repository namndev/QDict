...
public class M2tToast extends Toast {
	...
	public static M2tToast makeText(Context context, CharSequence text,
			int duration) {
		if (mInstance == null) {
		...
		tvMessage.setShadowLayer(SHADOW_TEXTVIEW, SHADOW_TEXTVIEW,
				SHADOW_TEXTVIEW, Color.BLACK);

		mToastYOffset = context.getResources().getDimensionPixelSize(
				R.dimen.toast_y_offset);

		mInstance.setView(v);
		mInstance.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM, 0,
				mToastYOffset);
	...
	public static M2tToast makeText(Context context, int resId, int duration)
	...
	public M2tToast setToastOnTop() {
		...
		mInstance.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.TOP, 0,
				mToastYOffset);
